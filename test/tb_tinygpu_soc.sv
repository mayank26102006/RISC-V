...
module tb_tinygpu_soc;
  ...
  tinygpu_soc #(
      ...
      // Not exercised by this testbench (which preloads memory via
      // hierarchical force) -- tied inactive.
      .ext_load_mode_i  (1'b0),
      .ext_load_bit_i   (1'b0),
      .ext_load_ready_o (),
  ...
  localparam int TEST1_LEN = 55;
    localparam logic [31:0] TEST1_PROG [0:255] = '{
      ...
      32'h0020f2b3, 32'h0020e333, 32'h0020c3b3, 32'h00c00413,
      32'h02819663, 32'h00000437, 32'hffe40413, 32'h02821c63,
      32'h00500413, 32'h04829463, 32'h00700413, 32'h04831c63,
      32'h00200413, 32'h06839463, 32'h07c0006f, 32'h90000737,
      32'h01470713, 32'hdead07b7, 32'h00178793, 32'h00f72023,
      32'h00100073, 32'h90000737, 32'h01470713, 32'hdead07b7,
      32'h00278793, 32'h00f72023, 32'h00100073, 32'h90000737,
      32'h01470713, 32'hdead07b7, 32'h00378793, 32'h00f72023,
      32'h00100073, 32'h90000737, 32'h01470713, 32'hdead07b7,
      32'h00478793, 32'h00f72023, 32'h00100073, 32'h90000737,
      32'h01470713, 32'hdead07b7, 32'h00578793, 32'h00f72023,
      32'h00100073, 32'h90000737, 32'h01470713, 32'hcafec7b7,
      32'habe78793, 32'h00f72023, 32'h00100073, 32'h00000000,
  ...
  localparam int TEST2_LEN = 58;
    localparam logic [31:0] TEST2_PROG [0:255] = '{
      ...
      32'h90000737, 32'h01470713, 32'hdead07b7, 32'h00178793,
      32'h00f72023, 32'h00100073, 32'h90000737, 32'h01470713,
      32'hdead07b7, 32'h00278793, 32'h00f72023, 32'h00100073,
      32'h90000737, 32'h01470713, 32'hdead07b7, 32'h00378793,
      32'h00f72023, 32'h00100073, 32'h90000737, 32'h01470713,
      32'hdead07b7, 32'h00478793, 32'h00f72023, 32'h00100073,
      32'h90000737, 32'h01470713, 32'hcafec7b7, 32'habe78793,
      32'h00f72023, 32'h00100073, 32'h00000000, 32'h00000000,
      32'h00000000, 32'h00000000, 32'h00000000, 32'h00000000,
      32'h00000000, 32'h00000000, 32'h00000000, 32'h00000000,
      32'h00000000, 32'h00000000, 32'h00000000, 32'h00000000,
      32'h00000000, 32'h00000000, 32'h00000000, 32'h00000000,
  ...
  localparam int TEST3_LEN = 60;
    localparam logic [31:0] TEST3_PROG [0:255] = '{
      ...
      32'h00f72023, 32'h00100073, 32'h90000737, 32'h01470713,
      32'hdead07b7, 32'h00478793, 32'h00f72023, 32'h00100073,
      32'h90000737, 32'h01470713, 32'hdead07b7, 32'h00578793,
      32'h00f72023, 32'h00100073, 32'h90000737, 32'h01470713,
      32'hcafec7b7, 32'habe78793, 32'h00f72023, 32'h00100073,
      32'h00000000, 32'h00000000, 32'h00000000, 32'h00000000,
      32'h00000000, 32'h00000000, 32'h00000000, 32'h00000000,
      32'h00000000, 32'h00000000, 32'h00000000, 32'h00000000,
      32'h00000000, 32'h00000000, 32'h00000000, 32'h00000000,
  ...
  // Machine-readable result, in addition to the printed summary below.
  // A cocotb test.py (or any other external harness) can poll
  // all_tests_done and then assert on all_tests_passed instead of having
  // to parse console text.
  logic all_tests_done   = 1'b0;
  ...
  // Manual enum-to-string conversion for trap_cause, used instead of the
  // built-in .name() method: Icarus fails at runtime on .name() when
  // called on a signal that arrived through a port connection ("must be
  // an enumeration variable, found a vpiNet"), even though it is
  // declared as trap_cause_e. This function is portable everywhere.
  function automatic string trap_cause_name(input trap_cause_e cause);
    case (cause)
      TRAP_NONE:              trap_cause_name = "TRAP_NONE";
      TRAP_ILLEGAL_INSTR:     trap_cause_name = "TRAP_ILLEGAL_INSTR";
      TRAP_INSTR_MISALIGNED:  trap_cause_name = "TRAP_INSTR_MISALIGNED";
      TRAP_LOAD_MISALIGNED:   trap_cause_name = "TRAP_LOAD_MISALIGNED";
      TRAP_STORE_MISALIGNED:  trap_cause_name = "TRAP_STORE_MISALIGNED";
      TRAP_BUS_ERROR:         trap_cause_name = "TRAP_BUS_ERROR";
      TRAP_ECALL:             trap_cause_name = "TRAP_ECALL";
      TRAP_EBREAK:            trap_cause_name = "TRAP_EBREAK";
      default:                trap_cause_name = "TRAP_UNKNOWN";
    endcase
  endfunction

  // Split into assert/release so memory can be loaded *while reset is
  // still held*, before the core ever attempts a fetch. Calling
  // load_program() between these two -- not after a combined
  // apply_reset() that already releases reset -- is required for
  // correctness: it is not simulator-scheduling-dependent this way.
  ...
  // Loads a program into instruction memory, selecting among the global
  // TESTn_PROG arrays by id (1..5) instead of taking an array as a task
  // parameter -- Icarus Verilog does not support array-typed subroutine
  // ports at all (fixed-size or open), so the array must be referenced
  // directly from module scope inside the task body instead of passed in.
  // Zeroes the rest of memory first so no stale content from a previous
  // test can ever be fetched.
  task automatic load_program(input int prog_id, input int len);
    int i;
    ...
    for (i = 0; i < len; i = i + 1) begin
      unique case (prog_id)
        1: dut.u_mem.mem_q[i] = TEST1_PROG[i];
        2: dut.u_mem.mem_q[i] = TEST2_PROG[i];
        3: dut.u_mem.mem_q[i] = TEST3_PROG[i];
        4: dut.u_mem.mem_q[i] = TEST4_PROG[i];
        5: dut.u_mem.mem_q[i] = TEST5_PROG[i];
        default: ;
      endcase
    end
  endtask
  ...
  task automatic print_status(input string tag);
    $display("    [%s] pc=0x%08x retire_count=%0d cpu_halted=%0d cpu_trap=%0d trap_cause=%s accel(busy=%0d done=%0d error=%0d)",
              ...
              trap_cause_name(trap_cause), accel_busy, accel_done, accel_error);
  endtask
  ...
  task automatic run_selfcheck_test(input string name, input int prog_id, input int len);
    bit timed_out;
    ...
    if (timed_out) begin
      ...
      $display("    RESULT: FAIL (unexpected trap, cause=%s)", trap_cause_name(trap_cause));
      record(name, 1'b0);
  ...
  task automatic run_illegal_instr_test();
    ...
    if (timed_out) begin
      ...
      $display("    RESULT: FAIL (expected cpu_trap=1, trap_cause=TRAP_ILLEGAL_INSTR; got cpu_trap=%0d trap_cause=%s)",
                cpu_trap, trap_cause_name(trap_cause));
      record("TEST5_illegal_instruction", 1'b0);
    end
  endtask
  ...
  initial begin
    ...
    run_selfcheck_test("TEST1_alu_regfile",     1, TEST1_LEN);
    run_selfcheck_test("TEST2_load_store",      2, TEST2_LEN);
    run_selfcheck_test("TEST3_branch_jump",     3, TEST3_LEN);
    run_selfcheck_test("TEST4_accelerator",     4, TEST4_LEN);
