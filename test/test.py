...
#
# Program (18 words, fits the 32-word ASIC-target scratchpad):
#   x1 = 5; x2 = 7; x3 = x1 + x2 (expect 12); compare; report PASS/FAIL via
#   the debug PASSFAIL register; EBREAK.
...
MINI_PROGRAM = [
    ...
    0x00c00a13,  # addi x20, x0, 12
    0x01419463,  # bne  x3, x20, fail1
    0x01c0006f,  # jal  x0, pass
    # fail1:
    0x90000f37,  # lui  x30, DBG_PASSFAIL(hi)
    0x014f0f13,  # addi x30, x30, DBG_PASSFAIL(lo)
    0xdead0fb7,  # lui  x31, FAIL(1)(hi)
    0x001f8f93,  # addi x31, x31, FAIL(1)(lo)
    0x01ff2023,  # sw   x31, 0(x30)
    0x00100073,  # ebreak
    # pass:
    0x90000f37,  # lui  x30, DBG_PASSFAIL(hi)
    0x014f0f13,  # addi x30, x30, DBG_PASSFAIL(lo)
    0xcafecfb7,  # lui  x31, PASS_SIGNATURE(hi)
    0xabef8f93,  # addi x31, x31, PASS_SIGNATURE(lo)
    0x01ff2023,  # sw   x31, 0(x30)
    0x00100073,  # ebreak
]
