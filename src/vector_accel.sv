...
module vector_accel (
  ...
  // Latched command/operands. Not used by the v1 (single-cycle) datapath
  // below -- kept for a future multi-cycle extension. See FIX NOTE above.
  accel_cmd_e  cmd_q;
  logic [31:0] src_a_q;
  logic [31:0] src_b_q;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      // Latch operands and command on start (for future multi-cycle use;
      // not read by the v1 combinational datapath above).
      if (accel_start_i) begin
        cmd_q   <= accel_cmd_i;
        src_a_q <= accel_src_a_i;
        src_b_q <= accel_src_b_i;
      end
...
`ifdef ASSERT_ON
  ...
  property p_result_matches_exec_on_start;
    @(posedge clk) disable iff (!rst_n)
    (accel_start_i && (accel_cmd_i inside {
        ACC_CMD_VADD8, ACC_CMD_VSUB8, ACC_CMD_VMAX8,
        ACC_CMD_RELU8, ACC_CMD_DOT4I8
