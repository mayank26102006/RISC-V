...
module debug_regs (
  ...
  // scr_addr_q/scr_wdata_q removed 2026-07-27 (64 bits) -- these were
  // already documented as non-functional stubs (never wired to real
  // scratchpad access). Reads/writes to those addresses now return/
  // accept a constant 0 with no real storage backing them.
  ...
  always_comb begin
    ...
    if (valid_i) begin
      unique case (word_off)
        ...
        WOFF_SCR_ADDR:     rdata_o = 32'd0; // scr_addr_q removed 2026-07-27, was never functional
        WOFF_SCR_WDATA:    rdata_o = 32'd0; // scr_wdata_q removed 2026-07-27, was never functional
    ...
    if (!rst_n) begin
      ...
      if (valid_i && we_i) begin
        unique case (word_off)
          ...
          // -----------------------------------------------------
          // DBG_REG_SCR_ADDR / SCR_WDATA: stubs, same as SCR_CONTROL.
          // No storage backs these (removed 2026-07-27, area reduction --
          // they were never functional). Flags debug_cmd_error instead of
          // silently accepting a write that changes nothing.
          // -----------------------------------------------------
          WOFF_SCR_ADDR:  debug_cmd_error_q <= 1'b1;
          WOFF_SCR_WDATA: debug_cmd_error_q <= 1'b1;
