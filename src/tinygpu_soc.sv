...
module tinygpu_soc #(
    ...
    // it gets this default). 32 words = 128 bytes shared instruction+
    // data memory, chosen from real LibreLane hardening data: a real
    // GDS run at 256 words showed 162.45% utilization at a 4x4 tile
    // grid (~26 tiles worth of area needed), and the math traces almost
    // all of that to flip-flop-based memory (256 words = 1024 bytes at
    // ~40 bytes/tile => ~25.6 tiles) -- confirming memory size, not CPU
    // logic, was the dominant area cost. tb_tinygpu_soc.sv explicitly
    // overrides this back up to 256 for simulation, since our test
    // programs need the extra room and simulation doesn't have a tile
    // budget -- so this default only affects the real chip.
    parameter int unsigned        SCRATCHPAD_WORDS   = 32
) (
    input  logic clk,
    input  logic rst_n,
