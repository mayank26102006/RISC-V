...
// -------------------------------------------------------------------
// Note: simulation memory preload
// -------------------------------------------------------------------
// This design's testbench (tb_tinygpu_soc.sv) preloads program memory by
// directly force-assigning mem_q[] hierarchically before releasing
// reset, rather than through a $readmemh-based INIT_FILE parameter --
// so no such parameter exists here. (An earlier version of this file
// had a `parameter string INIT_FILE` for that purpose, but Yosys's
// Verilog frontend -- used by the LibreLane hardening flow -- cannot
// parse a `string`-typed parameter in a module's parameter port list,
// even when its only use is inside an `ifdef`-guarded simulation-only
// block. Removed rather than worked around, since it wasn't needed.)
